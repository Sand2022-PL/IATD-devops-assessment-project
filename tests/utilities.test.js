...
//Test the two characters match first two letters of airline name provided
test("starts with first two letters of airline", () => {
  const id = generateFlightId("Qantas");
  expect(id.slice(0, 2)).toBe("QA");
});

//Test for too short or empty airlines names and return undefined
test("returns undefined for empty or short airline names", () => {
